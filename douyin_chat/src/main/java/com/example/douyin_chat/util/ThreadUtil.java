...
public class ThreadUtil {
    ...
    /**
     * 懒汉式单例创建线程池：用户CPU密集型任务
     */
    private static class CpuIntenseTargetThreadPoolLazyHolder{
        // 线程池: 用于CPU密集型任务
        private static final ThreadPoolExecutor EXECUTOR = new ThreadPoolExecutor(
                MAXIMUM_POOL_SIZE,  // 核心线程数
                MAXIMUM_POOL_SIZE,  // 最大线程数
                KEEP_ALIVE_SECONDS, // 空闲线程最大存活时间
                TimeUnit.SECONDS,   // 单位时间
                new LinkedBlockingDeque<>(QUEUE_SIZE),  // 任务队列
                new CustomThreadFactory("cpu") // 线程工厂
        );

        static{
            EXECUTOR.allowCoreThreadTimeOut(true);  //允许核心线程在空闲时超时终止
            // JVM 关闭时的钩子函数
    ...
    public static void sleepSeconds(int second) {
        LockSupport.parkNanos(second * 1000L * 1000L * 1000L);          // 当前线程阻塞
    }
