...
3. 视频发布模块 
   ...
   19. TODO: 点击视频列表文件后跳转到播放
   20. URL信息都可以进行缓存，特别是视频列表这些热点数据
   21. 视频整体多线程下载 
   22. 消息队列（预下载）
   23. 分布式文件系统
4. 视频流模块
   1. 根据Redis缓存一条一条获取media_url
   2. 在视频推荐栏中，先用推模式获取视频流demo   (根据视频ID查)
   3. 分页查询              (直接在Redis查，时间排序)
   4. 根据视频列表点击数据查询 (按照用户视频列表查询，时间排序)
   5. 预加载
   6. 关注 (推拉等模式)    (关注列表，投喂，消息队列)
   7. 推荐
